        ...
        # ...and it is defined: "load" those settings.
            ...
            if args.group is not None and args.group:
                    if args.group not in config["groups"]:
                        ...
                        # make a list for the user-supplied and valid group
                        user_envs = ",".join(config["groups"][args.group])
            else:
                # no group passed so get the environment
                user_envs = args.env
        # We now have a list of one or more user-defined environments
        for environment in user_envs.split(','):
            ...
            if environment in config["environments"]:
                ...
                print(
                    "The group contains an invalid environment: ", environment)
                sys.exit(1)
